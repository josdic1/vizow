...
beforeEach(() => {
  archivedAt = new Date("2026-08-06T14:00:00.000Z");
    ...
    if (
      ...
      return {
        rows: [{
          ...
          cycleCompletedAt: completedAt,
          cycleCreatedAt: createdAt,
          cycleUpdatedAt: completedAt,
        }],
      };
    }
...
async function archiveJob(): Promise<request.Response> {
  return request(app)
    .post(`/api/jobs/${jobId}/archive`)
    .send({});
}

describe("POST /api/jobs/:jobId/archive commit boundary", () => {
  it("builds a valid response before committing", async () => {
    const response = await archiveJob();
    ...
    expect(response.body).toMatchObject({
      ...
      job: {
        id: jobId,
        lifecycleStatus: "active",
        archivedAt: "2026-08-06T14:00:00.000Z",
        currentCycle: {
          id: cycleId,
          stage: "completed",
        },
