...
beforeEach(() => {
  unarchivedAt = new Date("2026-08-06T14:00:00.000Z");
...
async function unarchiveJob(): Promise<request.Response> {
  return request(app)
    .post(`/api/jobs/${jobId}/unarchive`)
    .send({});
}

describe("POST /api/jobs/:jobId/unarchive commit boundary", () => {
  it("builds a valid response before committing", async () => {
    const response = await unarchiveJob();
