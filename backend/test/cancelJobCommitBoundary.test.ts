...
const cancellationReason = "Client cancelled the project.";
...
beforeEach(() => {
  cancelledAt = new Date("2026-08-06T14:00:00.000Z");
  ...
  database.query.mockImplementation(async (sql: string) => {
    ...
    if (
      ...
      sql.includes("FOR UPDATE OF job, cycle")
    ) {
      return {
        rows: [{
          organizationId,
    ...
    if (sql.includes("WITH cancelled_visits AS")) {
      return { rows: [{ cancelledVisitCount: 1 }] };
    }
...
async function cancelJob(): Promise<request.Response> {
  return request(app)
    .post(`/api/jobs/${jobId}/cancel`)
    .send({ reason: cancellationReason });
}

describe("POST /api/jobs/:jobId/cancel commit boundary", () => {
  it("builds a valid response before committing", async () => {
    ...
    expect(response.body).toMatchObject({
      ...
      job: {
        id: jobId,
        lifecycleStatus: "cancelled",
        cancelledAt: "2026-08-06T14:00:00.000Z",
        cancellationReason,
        archivedAt: null,
        currentCycle: {
          id: cycleId,
